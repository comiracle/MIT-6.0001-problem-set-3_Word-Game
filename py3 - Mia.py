...
def update_hand(hand, word):
    ...
    returns: dictionary (string -> int)
    """
    new_hand = hand.copy()                                   #  iterate each letter in the passed word argument
                                                             #  passed word will always be in lower case
    for letter in word.lower():                              #  if letter in the given hand dictionary
        if letter in new_hand.keys():                      #  value for that letter will be deducted by one
            new_hand[letter] -= 1                          #  removes negative values in new_hand
    for letter in new_hand:
        if new_hand[letter] < 0:
            new_hand[letter] = 0                           #  return updated copy of hand dict
    return new_hand
...
def is_valid_word(word, hand, word_list):
    ...
    word = word.lower()
    #  new_hand dictionary to retain original values in hand dict
    new_hand = hand.copy()
    ...
    if word in word_list and word.find("*") == -1:           #  iterate each letter in word
        for letter in word:                                  #  chech if letter in hand dictionary
            if letter in hand:                              #  update values in new_hand by subtracting values of word_dict from values of new_hand
                new_hand[letter] = new_hand[letter] - word_dict[letter]       # if letter in word is not in hand dictionary, return False
            else:
                return False

    #  solution B block
    #  executes if passed word is not in word_list and contains a wildcard (*)
    elif word not in word_list and word.find("*") != -1:          #  any matched word between the passed word replaced with a vowel
                                                             #  and a word from the word_list will be appended to this list
        matched_word = []
        in_wordlist = False                                  #  iterate each vowel in VOWELS constant
        for vowel in VOWELS:                                #  replace '*' with a vowel
            word_replace = word.replace("*", vowel)            #  check if the passed replaced word is in word_list
            if word_replace in word_list:                       #  append the mached word in the matched_word list
                matched_word.append(word_list[word_list.index(word_replace)])       #  trigger execution of matched word
                in_wordlist = True                                  #  no matched word means the passed word is not in the word_list
        if len(matched_word) == 0:
            return False                                    #  executes if the word replaced with an actual vowel is in word_list
        if in_wordlist:
            for letter in word:
                if letter in hand:
                    new_hand[letter] = new_hand[letter] - word_dict[letter]     # if letter in word is not in hand dictionary, return False
                else:
                    return False                                        #  did not met above conditions(i.e. word not in word_list)
    else:
        return False
    ...
    for key in hand:
        if letter == key:
            if sub_letter not in hand:
                new_hand[sub_letter[0]] = hand[key]
                del(new_hand[letter])
